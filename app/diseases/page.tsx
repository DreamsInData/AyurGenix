...
import { BookOpen, Search, Loader2, AlertCircle, ArrowRight, ChevronDown, ChevronUp, Plus, X, Sparkles } from 'lucide-react';
...
interface DiseaseResult {
    ...
    herbs?: DiseaseHerb[];
    formulations?: DiseaseFormulation[];
    lifestyle?: string[];
    yoga?: string[];
...
// Helper for rendering Markdown content consistently
const MarkdownText = ({ content }: { content: string }) => {
    if (!content) return null;
    return (
        <div className="markdown-content" style={{ fontSize: '14px', color: '#a5d6a7', lineHeight: 1.6 }}>
...
export default function DiseasesPage() {
    ...
    const handleSearch = async (disease: string) => {
        ...
        if (res.success) {
            console.log("Raw Disease API Response:", res.content); // DEBUG LOG
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        // Last resort: extract JSON object via regex
            ...
            <div style={{ maxWidth: '800px', margin: '0 auto', padding: 'clamp(20px, 4vw, 40px) clamp(16px, 3vw, 24px)' }}>
                ...
                {result && !loading && (
                    <div className="animate-fade-in-up">
                        ...
                        {/* Root Causes Section */}
                        {(!!result.dosha_root_cause || !!result.nidana) && (
                            <div className="glass-card" style={{ padding: '20px', marginBottom: '16px' }}>
                                <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '1rem', marginBottom: '12px', color: '#40c4ff' }}>🧬 Ayurvedic Analysis</h3>

                                {!!result.dosha_root_cause && (
                                    <div style={{ marginBottom: '16px' }}>
                                        <h4 style={{ fontSize: '13px', color: '#66bb6a', fontWeight: 600, marginBottom: '6px' }}>Dosha Root Cause</h4>
                                        <MarkdownText content={result.dosha_root_cause} />
                                    </div>
                                )}

                                {!!result.nidana && (
                                    <div style={{ marginBottom: '16px' }}>
                                        <h4 style={{ fontSize: '13px', color: '#66bb6a', fontWeight: 600, marginBottom: '6px' }}>Nidana (Etiology)</h4>
                                        <MarkdownText content={result.nidana} />
                                    </div>
                                )}

                                {!!result.samprapti && (
                                    <div>
                                        <h4 style={{ fontSize: '13px', color: '#66bb6a', fontWeight: 600, marginBottom: '6px' }}>Samprapti (Pathogenesis)</h4>
                                        <MarkdownText content={result.samprapti} />
                                    </div>
                                )}
                            </div>
                        )}
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                                <div className="glass-card" style={{ padding: '20px' }}>
                                    ...
                                    {(result.diet.eat || []).map((f, i) => (
                                        <div key={i} style={{ padding: '3px 0' }}><MarkdownText content={`• ${f}`} /></div>
                                    ))}
                                </div>
                                <div className="glass-card" style={{ padding: '20px' }}>
                        ...
                        {Array.isArray(result.herbs) && (
                            <div className="glass-card" style={{ padding: '20px', marginBottom: '16px' }}>
                                ...
                                {result.herbs.map((h, i) => (
                                    <div key={i} style={{ padding: '12px 0', borderBottom: '1px solid rgba(46,204,113,0.06)', fontSize: '13px' }}>
                                        <div style={{ fontWeight: 600, color: '#e8f5e9', marginBottom: '4px' }}>{h.name}</div>
                                        <MarkdownText content={h.usage} />
                                        <div style={{ marginTop: '4px', fontSize: '12px', color: '#66bb6a' }}>
                                            <strong>Dosage:</strong> {h.dosage}
                                        </div>
                                        {h.contraindications && (
                                            <div style={{ marginTop: '4px', fontSize: '12px', color: '#ffab00' }}>
                                                <strong>Caution:</strong> {h.contraindications}
                                            </div>
                                        )}
                                    </div>
                                ))}
                        ...
                        {/* Formulations */}
                        {Array.isArray(result.formulations) && (
                            <div className="glass-card" style={{ padding: '20px', marginBottom: '16px' }}>
                                <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '0.95rem', marginBottom: '12px' }}>💊 Classical Formulations</h3>
                                {result.formulations.map((f, i) => (
                                    <div key={i} style={{ padding: '12px 0', borderBottom: '1px solid rgba(46,204,113,0.06)', fontSize: '13px' }}>
                                        <div style={{ fontWeight: 600, color: '#e8f5e9', marginBottom: '4px' }}>{f.name}</div>
                                        {f.source_text && <div style={{ fontSize: '11px', color: '#85c1e9', marginBottom: '4px' }}>Source: {f.source_text}</div>}
                                        <MarkdownText content={f.usage} />
                                    </div>
                                ))}
                            </div>
                        )}
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
                            {Array.isArray(result.lifestyle) && (
                                <div className="glass-card" style={{ padding: '20px' }}>
                                    ...
                                    {result.lifestyle.map((l, i) => (
                                        <div key={i} style={{ padding: '4px 0' }}><MarkdownText content={`• ${l}`} /></div>
                                    ))}
                                </div>
                            )}
                            {Array.isArray(result.yoga) && (
                                <div className="glass-card" style={{ padding: '20px' }}>
                                    ...
                                    {result.yoga.map((y, i) => (
                                        <div key={i} style={{ padding: '4px 0' }}><MarkdownText content={`• ${y}`} /></div>
                                    ))}
                                </div>
                            )}
                        </div>
                        ...
                        {/* Modern Research (New) */}
                        {!!result.modern_research && (
                            <div className="glass-card" style={{ padding: '20px', marginBottom: '16px', background: 'rgba(52, 152, 219, 0.08)', border: '1px solid rgba(52, 152, 219, 0.15)' }}>
                                <h3 style={{ fontFamily: 'Outfit, sans-serif', fontSize: '0.95rem', marginBottom: '8px', color: '#40c4ff' }}>🔬 Modern Research Insights</h3>
                                <MarkdownText content={result.modern_research} />
                            </div>
                        )}
